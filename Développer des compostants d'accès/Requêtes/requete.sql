...
SELECT commande_produit.id_produit AS "Référence du produit", description_court_produit AS "Nom du produit", quantiter AS "Quantité commandé"  
...
SELECT statut_client, nom_client AS "Nom du client", SUM((prix_total * quantiter) - coeff_client/100) AS "Chiffre d'affaire" FROM CLIENT 
JOIN commande ON CLIENT.id_client = commande.id_client 
JOIN commande_produit ON commande.id_commande = commande_produit.id_commande 
GROUP BY nom_client;
